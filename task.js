...
var test_in = "<p>You will now do several trials where you guess the number of "
test_in = test_in + "X's on the screen as quickly and accurately "
test_in = test_in + "as possible. You will earn money if you are faster and "
test_in = test_in + "more accurate, so try to balance them evenly. "
...
var Trial = function() {
	...
	function score(accuracy, RT) {
		...
		var b = 5
		var c = 5
		var s = 1/(1 + Math.exp(-b*accuracy - c*RT))
		return s;
	}
	...
	function runSmallTrial() {
		...
		if (((part.NAME == "test") | (trials < NUMTRIALS)) & !(begin)) {
			...
			if ((trials > 0) & part.NAME == "test") {
				time_lapsed = Math.round((new Date() - test_trials)/1000);
				time_remain = NUMTRIALS - time_lapsed;
	...
	function continueSmallTrial(square, which_part) {
		...
		function handleTick(event) {
			if ((trials > 0) & part.NAME == "test") {
				...
				//time_lapsed = Math.round((new Date() - test_trials)/1000);
				//time_remain = NUM_TEST - time_lapsed;
			...
			if (keys[13] & !xonscreen & listen) {
					...
					var time_out = 100;
					if (part.NAME == "test") {
						displ.text = "Bonus: " + theScore + "cents";
